...
RSpec.describe Symptom, type: :model do
  ...
  describe '#required_for_step?' do
    context 'when current_step is nil' do
      it 'returns true' do
        expect(symptom.required_for_step?('pain_location')).to be_truthy
      end
    end
    ...
    context 'when current_step is less than or equal to given step' do
      ...
      it 'returns true for a previous step' do
        expect(symptom.required_for_step?('pain_type')).to be_falsey
      end

      it 'returns false for a subsequent step' do
        expect(symptom.required_for_step?('pain_intensity')).to be_falsey
      end
    end
  end
  ...
  describe 'step checking methods' do
    ...
    it '#on_pain_location_step? returns true for pain_location step' do
      expect(symptom.on_pain_location_step?).to be_truthy
    end

    it '#on_pain_type_step? returns false for pain_location step' do
      expect(symptom.on_pain_type_step?).to be_falsey
    end

    it '#on_pain_intensity_step? returns false for pain_location step' do
      expect(symptom.on_pain_intensity_step?).to be_falsey
    end

    it '#on_pain_start_time_step? returns false for pain_location step' do
      expect(symptom.on_pain_start_time_step?).to be_falsey
    end

    it '#on_injury_related_step? returns false for pain_location step' do
      expect(symptom.on_injury_related_step?).to be_falsey
    end

    it '#on_generate_care_methods_step? returns false for pain_location step' do
      expect(symptom.on_generate_care_methods_step?).to be_falsey
    end
  end

  describe '#generate_care_methods' do
    let(:care_methods) { symptom.generate_care_methods.to_a }  # .to_a で配列に変換
